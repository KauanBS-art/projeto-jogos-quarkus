...
public class PlataformaResource {
    ...
    @POST
    @RolesAllowed("ADM")
    public Response incluir(@Valid PlataformaDTO dto, @Context UriInfo uriInfo) {
        PlataformaResponseDTO created = service.create(dto);
        return Response.created(
            UriBuilder.fromResource(PlataformaResource.class)
                .path(PlataformaResource.class, "buscarPorId")
                .build(created.id())
    ...
    @Path("/{id: \\d+}")
    @RolesAllowed("ADM")
    public Response alterar(@PathParam("id") Long id, @Valid PlataformaDTO dto) {
