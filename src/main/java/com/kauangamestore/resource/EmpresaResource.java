...
public class EmpresaResource {
    ...
    @POST
    @RolesAllowed("ADM")
    public Response incluir(@Valid EmpresaDTO dto, @Context UriInfo uriInfo) {
        EmpresaResponseDTO created = service.create(dto);
        return Response.created(
            UriBuilder.fromResource(EmpresaResource.class)
                .path(EmpresaResource.class, "buscarPorId")
                .build(created.id())
    ...
    @Path("/{id: \\d+}")
    @RolesAllowed("ADM")
    public Response alterar(@PathParam("id") Long id, @Valid EmpresaDTO dto) {
